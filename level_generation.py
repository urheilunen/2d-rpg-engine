    ...
    def corridor_between_points(self, x1, y1, x2, y2, join_type='either'):
        if x1 == x2 and y1 == y2 or x1 == x2 or y1 == y2:
            ...
            if join_type is 'either' and {0, 1}.intersection(
                ...
                join = 'bottom'
            elif not (not (join_type is 'either') or not {self.width - 1, self.width - 2}.intersection(
                    {x1, x2}) and not {self.height - 1, self.height - 2}.intersection({y1, y2})):
...
if 0:
    ...
    gen.gen_level()
    gen.gen_tiles_level()
    for i in range(len(gen.tiles_level)):
